...
public class PerfectLinks implements StubbornLinks.Listener {
    ...
    public PerfectLinks(int nodeId, int port, Listener listener,
                        Map<Integer, FairLossLinks.NodeAddress> nodeAddresses) {
        this.nodeId = nodeId;
        this.listener = listener;
        this.stubbornLinks = new StubbornLinks(nodeId, port, this, nodeAddresses);
    ...
    public void broadcast(MessageType type, byte[] payload) {
        // This will be useful for HotStuff
        // For now, we can broadcast to all nodes in the network
    }

    @Override
    public void onSLDeliver(int senderId, Message message, InetAddress senderAddress, int senderPort) {
        ...
        if (message.getType() == MessageType.ACK) {
            ...
            if (parts.length >= 2) {
                try {
                    ...
                    stubbornLinks.stopRetransmitting(originalSender, originalMsgId, senderId);
                } catch (NumberFormatException e) {
                    System.err.println("[PL] Invalid ACK payload: " + ackPayload);
                }
            }
            return;
        }
    ...
    private void sendAck(InetAddress address, int port, int originalSenderId, long originalMessageId) {
        long ackMsgId = messageIdCounter.incrementAndGet();
        String ackPayload = originalSenderId + "-" + originalMessageId;
        Message ack = new Message(nodeId, originalSenderId, ackMsgId, MessageType.ACK, ackPayload);
